...
COCKROACH_DB_URL = "postgresql://Samuel:$Web4now$03@windeee-cluster-27677.j77.aws-us-east-1.cockroachlabs.cloud:26257/defaultdb?sslmode=require"
DOWNLOAD_DIR = "/home/azureuser/wind_tunnel_pipeline"

def get_db_connection():
    """Establishes a secure connection to the CockroachDB serverless cloud cluster."""
    return psycopg2.connect(COCKROACH_DB_URL)


def init_cockroach_tables():
    """Verifies and constructs the cloud relational schemas if they do not exist."""
...
def parse_nist_filename(file_path):
    ...
    try:
        ...
        print(f"⚠️ Filename parsing error on {filename}: {e}")
        return None
...
def parse_single_hdf4_file(file_path):
    ...
    try:
        ...
        # --- 1. SPATIAL DATA FRAMING (Extracting true Face Number from Column 2) ---
        coords_matrix = hdf.select('Flat_Tap_Coordinates')[:] 
        tap_labels_spatial = coords_matrix[0, :].astype(int).astype(str)
        face_vector = coords_matrix[1, :].astype(int) 
...
def push_to_cockroach(hdf_filename, df_taps, df_pressure):
    ...
    try:
        ...
        # Spatial Taps Insertion (Includes face_no mapping field natively)
        df_taps['model_id'] = model_id
        tap_insert_query = "INSERT INTO taps (model_id, tap_no, face_no, x, y) VALUES %s RETURNING id, tap_no;"
        tap_values = [tuple(x) for x in df_taps[['model_id', 'tap_no', 'face_no', 'x', 'y']].to_numpy()]
...
if __name__ == "__main__":
    ...
    try:
        ...
        for index, target_url in enumerate(master_zip_urls, 1):
            ...
            print(f"Target Archive: {filename_zip}")
            print(f"==================================================================")
        ...
        print("\n🏆 GLOBAL DATA EXTRACTION COMPLETE! Entire cloud database successfully populated.")
